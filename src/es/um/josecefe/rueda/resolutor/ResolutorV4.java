...
public class ResolutorV4 extends Resolutor {
    ...
    private Map<Dia, List<AsignacionDiaV1>> solucionesCandidatas;
    ...
    private void inicializa() {
        ...
        for (int indDia = 0; indDia < dias.length; indDia++) {
            Dia d = dias[indDia];
            ArrayList<AsignacionDiaV1> solucionesDia = new ArrayList<>();
            ...
            for (List<Set<Participante>> condDia : combinarConductoresDia) {
                ...
                if (nParticipantesIda.entrySet().stream().allMatch(e -> plazasIda.getOrDefault(e.getKey(), 0) >= e.getValue())
                    ...
                    List<Iterable<Lugar>> posiblesLugares = Stream.of(participantes).map(Participante::getPuntosEncuentro).
                    ...
                    for (List<Lugar> selLugares : new Combinador<>(posiblesLugares)) {
                        ...
                        Iterator<Lugar> il = selLugares.subList(participantes.length, selLugares.size()).iterator();
                        for (int i = 0; i < participantes.length; i++) {
                            lugaresIda.put(participantes[i], selLugares.get(i));
                            lugaresVuelta.put(participantes[i], selCond.contains(participantes[i])
                        ...
                        if (plazasNecesariasIda.entrySet().stream().allMatch(e -> e.getValue().entrySet().stream().allMatch(ll -> ll.getValue() <= plazasDisponiblesIda.get(e.getKey()).getOrDefault(ll.getKey(), 0)))
                            ...
                            // Calculamos coste
                            int coste = Stream.of(participantes).mapToInt(e -> e.getPuntosEncuentro().indexOf(lugaresIda.get(e)) + e.getPuntosEncuentro().indexOf(lugaresVuelta.get(e))).sum();// + selCond.size()*2;
                    ...
                    if (mejorCoste < Integer.MAX_VALUE) {
                        ...
                        solucionesDia.add(new AsignacionDiaV1(selCond, mejorLugaresIda, mejorLugaresVuelta, mejorCoste));
    ...
    public Map<Dia, ? extends AsignacionDia> resolver(Set<Horario> horarios) {
        ...
        System.out.println("====================");
        System.out.format("C=%.0f, ", C);
        System.out.println(estadisticas);
        System.out.println("----------------------------------------------------");

        // Construimos la solución final
        if (mejor.compareTo(RAIZ) < 0) {
            Iterator<AsignacionDiaV1> i = mejor.getEleccion().iterator();
    ...
    final private class Nodo implements Comparable<Nodo> {
        ...
        private final List<AsignacionDiaV1> eleccion;
        ...
        Nodo(Nodo padre, AsignacionDiaV1 nuevaAsignacion) {
            ...
            for (int i = 0; i < participantes.length; i++) {
                ...
                if (nuevaAsignacion.getConductores().contains(participantes[i])) {
                    if (++vecesConductor[i] > maximo) {
                        maximo = vecesConductor[i];
                    }
                }

                if (vecesConductor[i] < minimo) {
                    minimo = vecesConductor[i];
                }
            }
            if (isTerminal()) {
        ...
        List<AsignacionDiaV1> getEleccion() {
            return eleccion;
        }
